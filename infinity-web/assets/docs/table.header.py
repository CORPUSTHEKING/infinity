...
def backup_path_for(original: Path) -> Path:
    ...
    return backup_dir / f"{original.name}-dated.md"


def process_file(path: Path, output_path: Path | None):
    print(f"[1/3] copying {path.name} -> {path.stem}-table.md")
    table_copy = path.with_name(f"{path.stem}-table.md")
    shutil.copy2(path, table_copy)

    print(f"[2/3] moving {path.name} -> ~/backup/{path.name}-dated.md")
    backup_path = backup_path_for(path)
    shutil.move(str(path), str(backup_path))

    print(f"[3/3] transforming {table_copy.name} -> {path.name}")
    text = table_copy.read_text(encoding="utf-8")
    converted = convert_text(text)

    final_output = output_path if output_path is not None else path
    final_output.write_text(converted, encoding="utf-8")

    if table_copy.exists():
        table_copy.unlink()
